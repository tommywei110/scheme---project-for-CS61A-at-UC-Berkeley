...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        try:
            procedure = scheme_eval(first,env) # 1. evaluate the operator
            return procedure.eval_call(rest,env)
        except:
            raise SchemeError('Wrong Procedure {} in {}'.format(expr,env))
        # END PROBLEM 5
...
def scheme_apply(procedure, args, env):
    ...
    result = procedure.apply(args, env)
    return result
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        args = operands.map(lambda operand: scheme_eval(operand,env)) #2. evaluate the operand
        return scheme_apply(self,args,env) # 3. Apply the procedure on the evaluated operands.
        # END PROBLEM 5
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Illegal amount of arguments {0} with function {1}'.format(python_args,self.fn))
        # END PROBLEM 4
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        try:
            result = scheme_apply(self,operands,env)
        except:
            raise SchemeError('Failure to apply macro procedure on operands')
        try:
            scheme_expr = complete_eval(result)
        except:
            raise SchemeError('Failure to convert generate Scheme expressions')
        return scheme_eval(scheme_expr,env)
        # END Problem 21
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('MacroError non-symbol: {0}'.format(bad_target))
    # END Problem 21
